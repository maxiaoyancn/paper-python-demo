...
脚本 SHALL 在第 1 行写入每组数据的列序号（从 1 起），且 SHALL 把每组**最后一个数据列**的表头覆写为带样本量标注的形式：第 i 组写 `<N_i>（G<i>）`，使用全角中文括号。所有 K（包括 K=2）SHALL 统一使用 `Gi` 格式（不再使用 v1 的 `N` / `M`）。

#### Scenario: K=2 时第 1 组 N=10
- **WHEN** K=2、第 1 组样本量为 10
- **THEN** 第 1 行第 1 组所占的 10 列依次写入 `1, 2, …, 9, "10（G1）"`

#### Scenario: K=2 时第 2 组 M=12
- **WHEN** K=2、第 2 组样本量为 12
- **THEN** 第 1 行第 2 组所占的 12 列依次写入 `1, 2, …, 11, "12（G2）"`

#### Scenario: K=3 时第 3 组样本量
- **WHEN** K=3、第 3 组样本量为 15
- **THEN** 第 1 行第 3 组所占的 15 列依次写入 `1, 2, …, 14, "15（G3）"`
...
**Reason**：v2 把"两组场景的单一 p 值"扩展为多组流水线（Levene → Shapiro-Wilk → 整体 ANOVA/Kruskal-Wallis → 两两 Tukey/Welch+Bonferroni），不再适合用单条 requirement 表达。

**Migration**：参见本文件 `## ADDED Requirements` 的四条新需求：「Levene 方差齐性检验」、「Shapiro-Wilk 正态性检验」、「整体差异 p 值（ANOVA / Kruskal-Wallis）」、「两两对比 p 值与多重比较校正」。原 v1 的"K=2 + 方差齐 → Student t / 方差不齐 → Welch t"行为在 v2 等价于：「方差齐 → ANOVA（K=2 时与 Student t 双尾等价）」+「方差不齐 → KW（K=2 时与 Mann-Whitney U 等价）」+「两两对比恒为 1 对，按齐/正态决定 Tukey 或 Welch」。

### Requirement: 写入回算统计量到 AG-AK 风格列

**Reason**：v2 的统计列数量与含义完全重组：从 v1 的 5 列（μ1、σ1、μ2、σ2、p）变为 2K + 3 + 2·C(K,2) 列；列号布局也不再以 AG 起。

**Migration**：参见本文件 `## ADDED Requirements` 的「按统计列布局写入回算结果」与四条统计需求。新布局起始列 = `数据区结束列 + 2`，写入顺序固定为：μ_1、σ_1、…、μ_K、σ_K、Levene_p、Shapiro_min_p、Overall_p、Pairwise_raw_p × C(K,2)、Pairwise_Q × C(K,2)。
...
#### Scenario: K=3 时 raw 与 Q 列顺序
- **WHEN** K=3
- **THEN** 共 6 列：依次为 raw_(1,2)、raw_(1,3)、raw_(2,3)、Q_(1,2)、Q_(1,3)、Q_(2,3)
...
脚本 SHALL 在数据区结束后空 1 列，从下一列起按以下顺序连续写入统计列（K 组场景）：μ_1、σ_1、μ_2、σ_2、…、μ_K、σ_K、Levene p、Shapiro-Wilk min p、整体 p、Pairwise raw p × C(K,2)、Pairwise Q-value × C(K,2)。所有数值 SHALL 保留 4 位小数。脚本 SHALL 在第 1 行同时写入对应的中文表头：`第i组均值`、`第i组SD值`、`Levene p`、`Shapiro-Wilk min p`、`整体 p（ANOVA/KW）`、`G<i>-G<j> raw p`、`G<i>-G<j> Q-value`。如果第 1 行某统计列已有非空表头，SHALL 保留不覆盖。

#### Scenario: K=2 时统计列总数
- **WHEN** K=2
- **THEN** 统计列共 9 列：μ1、σ1、μ2、σ2、Levene、SW、整体 p、G1-G2 raw、G1-G2 Q

#### Scenario: K=3 时统计列总数
- **WHEN** K=3
- **THEN** 统计列共 15 列：μ1、σ1、μ2、σ2、μ3、σ3、Levene、SW、整体 p、G1-G2 raw、G1-G3 raw、G2-G3 raw、G1-G2 Q、G1-G3 Q、G2-G3 Q

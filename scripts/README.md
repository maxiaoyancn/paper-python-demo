...
- 每个数据列的第 1 行表头被覆写为 `<letter><idx>`：第 i 组用第 i 个英文大写字母（i=1..K → A..Z），idx 从 1 起递增到 N_i。例如 K=2、N_1=10、N_2=12 时，第 1 组列表头依次为 `A1, A2, ..., A10`，第 2 组为 `B1, B2, ..., B12`。**K 上限 26**（K>26 时脚本退出码 3）。

**统计区**（脚本写入，紧跟数据区 + 1 空列）：

| 列 | 表头 | 含义 |
|---|---|---|
| 2K 列 | `A 组均值` / `A 组SD值` / `B 组均值` / `B 组SD值` / … | 每组的均值与 SD，4 位小数 |
| 1 列 | `Levene p` | Brown-Forsythe Levene（center=median）；> 0.05 视为方差齐 |
| 1 列 | `是否方差齐` | Y/N 结论列：Y = Levene p > 0.05；N 反之 |
| 1 列 | `Shapiro-Wilk min p` | 每组 SW 取最小 p；> 0.05 视为全部正态 |
| 1 列 | `是否正态` | Y/N 结论列：Y = SW min p > 0.05；N 反之 |
| 1 列 | `整体 p（ANOVA/KW）` | 齐 → ANOVA；不齐 → Kruskal-Wallis |
| C(K,2) 列 | `A-B raw p` / `A-C raw p` / … | 两两 raw p；按字典序 (1,2)(1,3)(2,3)... |
| C(K,2) 列 | `A-B Q-value` / … | 校正后 p：齐+全正态走 Tukey 时留空；否则为 Welch+Bonferroni 校正值 |

**示例**：

| K | N 列表 | 位数列 | 数据区 | 统计区起 | 统计列数 |
|---|---|---|---|---|---|
| 2 | (10, 12) | H (8) | J-S, U-AF | AH (34) | **11** |
| 3 | (8, 10, 12) | K (11) | M-T, V-AE, AG-AR | AT (46) | **17** |
| 4 | (6, 8, 10, 12) | N (14) | P-U, W-AD, AF-AO, AQ-BB | BD (56) | **25** |
